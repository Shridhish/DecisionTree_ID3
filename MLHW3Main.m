%#####################################################################
% Group : Babu,Sundaran,Vemparala,Vincent
% #M - Students Name
% M12506236 - Nithyasri Babu
% M12484516 - Roshan Joe Vincent
% M12484014 - Sahiti Vemparala
% M12494917 - Shridirsh Ramnath Sundaran
%##################################################################### 
%#####################################################################
...
for j=5:5:20
    ...
    AccuraciesNBC_EachBin = zeros(RunningTime, 1);
    ...
    for i=1:RunningTime
        ...
        AccuraciesNBC_EachBin(i) = AccuracyNBC; 
    end
    ...
    AccuraciesNBC(j/5, MIN_ACCURACY) = min(AccuraciesNBC_EachBin);
    AccuraciesNBC(j/5, MAX_ACCURACY) = max(AccuraciesNBC_EachBin);
    AccuraciesNBC(j/5, AVG_ACCURACY) = mean(AccuraciesNBC_EachBin);
end
...
%tree results
hFig1 = figure('Name', 'Min/Max/Avg accuracies for Decision Tree at different bin numbers (10 runs)');
%subplot(2,2,1);
...
%NBC results
hFig2 = figure('Name', 'Min/Max/Avg accuracies for Naive Bayes Classifier at different bin numbers (10 runs)');
%subplot(2,2,2);
